...
$site_url    = site_url();
$filter_url  = site_url(SITE_AREA . '/laporan-database/filter');
$export_url  = ($page_type === 'pdf')
    ? site_url(SITE_AREA . '/laporan-database/cetak-pdf')
    : site_url(SITE_AREA . '/laporan-database/cetak-excel');
$export_id   = ($page_type === 'pdf') ? 'btn-pdf' : 'btn-excel';
$reset_url   = site_url(SITE_AREA . '/laporan-database/' . $page_type);
...
<div class="row">
    <div class="col-12">
        ...
        <?php echo $this->load->view('reports/_data_database', array(
            'rows'        => $rows,
            'tgl_mulai'   => $tgl_mulai,
            'tgl_akhir'   => $tgl_akhir,
