<?php
$show_pdf   = isset($show_pdf)   ? $show_pdf   : false;
$show_excel = isset($show_excel) ? $show_excel : true;
...
<div class="card" id="card-data">
    <div class="card-header">
        <h3 class="card-title">Riwayat Backup Database — Periode: <?php echo html_escape($periode_label); ?></h3>
        <span class="float-right">Jumlah: <?php echo count($rows); ?> backup</span>
    </div>
    <div class="card-body table-responsive">
        <?php if (empty($rows)) : ?>
            <div class="alert alert-warning mb-0"><i class="fas fa-exclamation-triangle"></i> Tidak ada riwayat backup database pada periode yang dipilih.</div>
        <?php else : ?>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <?php if ($r->status === 'Berhasil') : ?>
                                <span class="badge badge-success"><?php echo html_escape($r->status); ?></span>
                            <?php else : ?>
                                <span class="badge badge-danger"><?php echo html_escape($r->status); ?></span>
                            <?php endif; ?>
                        </td>
