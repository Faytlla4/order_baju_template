...
class Master extends App_Controller
{
	public function __construct()
	{
		...
		Assets::add_module_js('master_ukuran', 'master_ukuran.js');
	}

	public function index()
	{
		Template::set('toolbar_title', lang('master_ukuran_manage'));
		Template::render();
	}
	...
	public function create()
	{
		if (isset($_POST['save'])) {
			...
			if (!empty($this->master_ukuran_model->error)) {
				Template::set_message(lang('master_ukuran_create_failure') . $this->master_ukuran_model->error, 'error');
			}
		}
		...
		Template::set('toolbar_title', lang('master_ukuran_action_create'));
		Template::render();
	}
	...
	public function edit()
	{
		...
		Template::set('master_ukuran', $this->master_ukuran_model->find($id));
		Template::set('toolbar_title', lang('master_ukuran_edit_heading'));
		Template::render();
	}

	private function save_master_ukuran($type = 'insert', $id = 0)
	{
		if ($type == 'update') {
			$_POST['id'] = $id;
		}

		$this->form_validation->set_rules($this->master_ukuran_model->get_validation_rules());
		if ($this->form_validation->run() === false) {
			return false;
		}

		$data = $this->master_ukuran_model->prep_data($this->input->post());
		...
		if ($type == 'insert' && trim((string) $target) !== '') {
			if (!$this->validasi_order_target($target)) {
				$this->master_ukuran_model->error = 'Order tidak dapat diproses karena master terkait berstatus Non Aktif.';
				return false;
			}
		}
		...
		if ($type == 'insert') {
			$data['urutan'] = $this->master_ukuran_model->get_next_urutan();
		} else {
			unset($data['urutan']);
		}
		...
		$return = false;
		if ($type == 'insert') {
			$id = $this->master_ukuran_model->insert($data);

			if (is_numeric($id)) {
				$return = $id;

				// Lengkapi order yang dipilih user (no INSERT order baru)
				$this->lengkapi_order('ukuran_id', $id, $this->input->post('kode_order'));
			}
		} elseif ($type == 'update') {
			$return = $this->master_ukuran_model->update($id, $data);
		}
	 ...
	 * (status = 1 dan deleted = 0). Bila order masih kosong (belum ada master)
	 * dianggap aman.
	 *
	 * @param int $order_id
	 *
	...
	public function get_data()
	{
		...
		if (!empty($return['data']) && is_array($return['data'])) {
			...
			$link = array();
			$orders = $this->db->select('ukuran_id, kode_order, nama_customer')
				->where('ukuran_id IS NOT NULL')
				->get('order_baju')
				->result();
			foreach ($orders as $o) {
				if (!isset($link[$o->ukuran_id])) {
					$link[$o->ukuran_id] = array('kode' => $o->kode_order, 'customer' => $o->nama_customer);
				}
			}
	...
	private function hapus_master_cascade($master_table, $fk_field, $master_id)
	{
		...
		if ($master_id <= 0) {
			$this->master_ukuran_model->error = 'Data master tidak ditemukan.';
			return false;
		}
		...
		if ($this->db->trans_status() === false) {
			$this->master_ukuran_model->error = 'Terjadi kesalahan saat menghapus data.';
			return false;
		}
