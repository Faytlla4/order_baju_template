...
$report_pdf_url = site_url(SITE_AREA . '/reports/report_pdf');
$report_pdf_pdf_url = site_url(SITE_AREA . '/reports/report_pdf/pdf');
$report_pdf_filter_url = site_url(SITE_AREA . '/reports/report_pdf/filter');
$inline_js = "
$(function() {
    ...
    var baseUrl = '{$report_pdf_url}';
    var filterUrl = '{$report_pdf_filter_url}';
    ...
    // Refresh setelah kedua tanggal native dipilih.
    $(document).on('change', '#tgl_mulai, #tgl_akhir', function() {
        if (document.getElementById('periode') && document.getElementById('periode').value === 'custom') {
            maybeRefresh();
        }
    });

    // Delegasi: tombol PDF bisa diganti oleh refresh AJAX.
    $(document).on('click', '#btn-pdf', function(e) {
        ...
        window.open('{$report_pdf_pdf_url}' + '?' + params, '_blank');
    });
});
";
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-file-pdf text-danger"></i> LAPORAN TRANSAKSI PDF</h3>
            </div>
            <div class="card-body">
                <div class="row align-items-end">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="dp_mulai" data-target-input="nearest">
                                        <input type="date" name="tgl_mulai" id="tgl_mulai" class="form-control" value="<?php echo html_escape($tgl_mulai); ?>" />
                                        <div class="input-group-append date-filter-trigger" data-date-input="tgl_mulai">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <div class="input-group date" id="dp_akhir" data-target-input="nearest">
                                        <input type="date" name="tgl_akhir" id="tgl_akhir" class="form-control" value="<?php echo html_escape($tgl_akhir); ?>" />
                                        <div class="input-group-append date-filter-trigger" data-date-input="tgl_akhir">
                                            <div class="input-group-text"><i class="far fa-calendar-alt"></i></div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="col-md-3">
                        <div class="d-flex align-items-end">
                            <a href="<?php echo site_url(SITE_AREA . '/reports/report_pdf'); ?>" class="btn btn-secondary"><i class="fas fa-undo"></i> Reset</a>
                        </div>
                    </div>
                </div>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">Riwayat Laporan PDF</h3>
            </div>
            <div class="card-body table-responsive pt-0">
                <?php if (empty($history_list)) : ?>
                    <div class="alert alert-info mb-0"><i class="fas fa-info-circle"></i> Tidak ada riwayat laporan PDF.</div>
                <?php else : ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($h->nama_file)) : ?>
                                        <div class="btn-group">
                                            <a href="<?php echo site_url(SITE_AREA . '/reports/report_pdf/view/' . (int) $h->id); ?>" class="btn btn-sm btn-outline-danger" target="_blank" title="Lihat PDF">
                                                <i class="fas fa-file-pdf"></i> PDF
                                            </a>
                                            <a href="<?php echo site_url(SITE_AREA . '/reports/report_pdf/download/' . (int) $h->id); ?>" class="btn btn-sm btn-outline-secondary" title="Download PDF">
